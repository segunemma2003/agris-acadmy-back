...
class UssdController extends Controller
{
     ...
     *     description="Handles USSD session callbacks from Africa's Talking. Requires HMAC signature validation via X-AfricasTalking-Hash header. Menu tree: language (English/Hausa) -> Main Menu with 1) Browse Courses (anonymous lead capture), 2) Talk to Facilitator, 3) Register via SMS link, 4) My Courses (Stage 03 Learn & Assess: Main -> My Courses -> [Course] -> Check my progress shows 'Module X of Y complete' then Mark today's lesson done; also Continue course / Contact facilitator; enrolled learner matched by calling phoneNumber against User.phone — no PIN step), 5) Log Apprenticeship ('Log today? 1=Yes, 2=No' + an optional short note, for a learner with an active/accepted apprenticeship placement). 'My Courses' writes to the same student_progress/enrollments tables as the web and mobile apps, so completions show up immediately on the web dashboard; 'Log Apprenticeship' writes to the same apprenticeship_logs table as the web daily-log form. Sessions are tracked server-side and end gracefully with a 'Session ended' message after 90s of inactivity; the Africa's Talking USSD channel's own session-timeout setting (dashboard-configured) should also be set to 90s so the carrier-level cutoff matches.",
