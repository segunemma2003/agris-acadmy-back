...
class CourseController extends Controller
{
    ...
    public function modules(Request $request, Course $course)
    {
        ...
        $modules->each(function ($module) use ($user) {
            ...
            $module->quiz_status = $module->quizStatusFor($user);
            // Flat flags for clients that only need pass/fail for next-lesson gating.
            $module->quiz_passed = (bool) ($module->quiz_status['quiz_passed'] ?? false);
            $module->quiz_completed = (bool) ($module->quiz_status['quiz_completed'] ?? false);
