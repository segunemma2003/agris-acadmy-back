...
use App\Filament\Tutor\Resources\TopicTestAttemptResource\Pages;
...
class TopicTestAttemptResource extends Resource
{
    protected static ?string $model = TopicTestAttempt::class;
    ...
    protected static ?string $navigationLabel = 'Topic Test Attempts';
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Test Information')
                    ->schema([
                        Forms\Components\Select::make('topic_test_id')
                            ->label('Topic Test')
                            ->relationship('topicTest', 'title', fn ($query) => $query->where('tutor_id', Auth::id()))
                            ->required()
    ...
    {
        return $table
            ->modifyQueryUsing(fn ($query) => $query->with(['topicTest', 'user'])
                ->whereHas('topicTest', fn ($q) => $q->where('tutor_id', Auth::id())))
            ->columns([
                Tables\Columns\TextColumn::make('topicTest.title')
                    ->label('Test')
                    ->searchable()
                    ->sortable()
                    ->wrap(),
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('topic_test_id')
                    ->label('Test')
                    ->relationship('topicTest', 'title', fn ($query) => $query->where('tutor_id', Auth::id()))
                    ->searchable()
                    ->preload(),
    ...
    {
        return [
            'index' => Pages\ListTopicTestAttempts::route('/'),
            'view' => Pages\ViewTopicTestAttempt::route('/{record}'),
        ];
    }
